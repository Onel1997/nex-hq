...
import { buildAnalyticsChamberPayload } from "@/lib/facility/analytics-intelligence";
...
export async function GET() {
  try {
    ...
    const payload = await buildAnalyticsChamberPayload(snapshot, tasks, brainReports);

    return NextResponse.json(payload);
  } catch (error) {
    const message =
      error instanceof Error ? error.message : "Failed to load Analytics Chamber";

    console.error("[Analytics Chamber]", message, error);
