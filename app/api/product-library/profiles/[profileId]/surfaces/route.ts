...
import { saveProductPrintSurface, toOwnerProductProfileView } from "@/lib/product-library/service";
...
export async function POST(request: Request, context: { params: Promise<{ profileId: string }> }) {
  ...
  try {
    ...
    const profile = await saveProductPrintSurface(gated.scope, profileId, await request.json());
