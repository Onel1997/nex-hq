...
async function parseReviewBody(request: Request): Promise<ReviewReportOptions> {
  try {
    ...
    // Empty body is valid for approve.
  }
  return {};
}
...
async function runReviewRoute(
  ...
  try {
    ...
    const result = await reviewReportRecord(id, action, options);
...
export async function POST(
  ...
  return runReviewRoute(request, context, "approve");
}
