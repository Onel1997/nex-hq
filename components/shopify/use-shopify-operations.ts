...
export function useShopifyOperations() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      let historyBody: (CommerceHistoryResponse & { error?: string }) | null = null;
      if (historyResponse.ok) {
        historyBody = (await historyResponse.json()) as CommerceHistoryResponse & {
          error?: string;
        };
      }
      ...
      setData({
        ...
        marketPrintIntelligence:
          body.marketPrintIntelligence ?? emptyMarketPrintIntelligence(),
        commerceHistory:
          historyBody &&
