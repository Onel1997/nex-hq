...
async function buildLiveFacilitySnapshot(): Promise<FacilitySnapshot> {
...
export async function getFacilitySnapshot(): Promise<FacilitySnapshot> {
  const result = await getFacilitySnapshotResult();
  return result.snapshot;
}

export interface FacilitySnapshotResult {
  snapshot: FacilitySnapshot;
  quotaDegraded: boolean;
}

export async function getFacilitySnapshotResult(): Promise<FacilitySnapshotResult> {
  try {
    return {
      snapshot: await buildLiveFacilitySnapshot(),
      quotaDegraded: false,
    };
  } catch (error) {
    if (isRecoverableFacilityError(error)) {
      console.warn("[Facility] Running in local fallback mode:", error);
      return {
        snapshot: buildFallbackFacilitySnapshot(),
        quotaDegraded: isProviderQuotaError(error),
      };
    }
    throw error;
  }
