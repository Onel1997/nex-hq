...
const KNOWLEDGE_STREAM_DEFS: Array<Omit<BrainCoreKnowledgeStream, "active" | "signalCount">> = [
  { id: "stream-ceo-research", from: "CEO Command", to: "Research HQ" },
  { id: "stream-research-commerce", from: "Research HQ", to: "Commerce Lab" },
  { id: "stream-commerce-design", from: "Commerce Lab", to: "Design Studio" },
  { id: "stream-commerce-shopify", from: "Commerce Lab", to: "Shopify Operations" },
  { id: "stream-design-content", from: "Design Studio", to: "Content Studio" },
  { id: "stream-design-image", from: "Design Studio", to: "Image Studio" },
  { id: "stream-image-content", from: "Image Studio", to: "Content Studio" },
  { id: "stream-content-marketing", from: "Content Studio", to: "Marketing Center" },
  { id: "stream-content-image", from: "Content Studio", to: "Image Studio" },
  { id: "stream-shopify-research", from: "Shopify Operations", to: "Research HQ" },
];
...
function buildKnowledgeStreams(
  ...
  const hasContent = reports.some((r) => r.category === "content");

  const streamActivity: Record<string, { active: boolean; count: number }> = {
    "stream-ceo-research": {
      active: ceoActive || hasResearch,
      count: (ceoActive ? 1 : 0) + reports.filter((r) => r.category === "research").length,
    },
    ...
    "stream-commerce-design": {
      active: hasCommerce && (hasDesign || snapshot.labs.designer.opsState !== "idle"),
      count: reports.filter((r) => r.category === "commerce" || r.category === "design").length,
    },
    ...
    "stream-content-image": {
      active: hasContent || reports.some((r) => r.category === "image"),
      count: reports.filter((r) => r.category === "content" || r.category === "image").length,
    },
    "stream-shopify-research": {
      active: hasResearch || snapshot.labs.shopify.opsState !== "idle",
