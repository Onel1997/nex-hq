...
function agentTabForReport(
  report: ReportListItem,
  type: ReportsCenterType,
): ReportsCenterAgentTab {
  if (report.agentTab) return report.agentTab;
  if (type === "mission_summary") {
    return agentTabForAgentId(report.agentId);
  }
  if (type === "ceo_briefing") return "ceo";
...
function buildTags(report: ReportListItem): string[] {
  ...
  if (report.source) tags.add(report.source);
...
function reportToCenterReport(
  report: ReportListItem,
  tasks: TaskListItem[],
  overrides?: Partial<Pick<ReportsCenterReport, "source" | "agentTab" | "isCeoBriefing">>,
): ReportsCenterReport {
  const type = reportType(report);
  const source = overrides?.source ?? report.source ?? "legacy";
  const agentTab = overrides?.agentTab ?? agentTabForReport(report, type);
...
function taskToMissionSummary(task: TaskListItem): ReportsCenterReport | null {
  ...
  return {
    ...
    tags: ["mission", "legacy", task.status === "completed" ? "complete" : "review"],
    type: "mission_summary",
    source: "legacy",
    agentTab: agentTabForAgentId(agentId),
    preview: {
...
async function buildCommerceReports(): Promise<ReportsCenterReport[]> {
  try {
    ...
    const source: ReportsCenterSource = "live";
...
function buildCeoBriefings(
  liveReports: ReportsCenterReport[],
  snapshot: FacilitySnapshot,
): ReportsCenterReport[] {
  const research = liveReports.filter((r) => r.agentTab === "research");
  const design = liveReports.filter((r) => r.agentTab === "design");
  const marketing = liveReports.filter((r) => r.agentTab === "marketing");
  const commerce = liveReports.filter((r) => r.agentTab === "commerce");
...
function buildDemoReports(tasks: TaskListItem[]): ReportsCenterReport[] {
  return MOCK_REPORTS.map((report) =>
    reportToCenterReport(
      { ...report, source: "demo" },
      tasks,
      { source: "demo" },
    ),
...
function buildActivityFeed(
  ...
  for (const report of reports.filter((r) => r.source === "live" && r.status === "approved").slice(0, 4)) {
    fromEvents.push({
...
function buildAgentTabFilters(reports: ReportsCenterReport[]): ReportsCenterAgentTabFilter[] {
  const tabs: ReportAgentTab[] = [
    "research",
    "design",
    "marketing",
    "commerce",
  ...
  return [
    ...
    ...tabs.map((id) => ({
      id,
      label: REPORT_AGENT_TAB_LABELS[id],
      count: reports.filter((r) => r.agentTab === id).length,
    })),
  ];
}
...
function buildSourceFilters(
  ...
  const all = [...liveReports, ...legacyArchive];
  const sources: ReportsCenterSource[] = ["live", "seed", "demo", "legacy"];

  return [
    { id: "live", label: REPORT_SOURCE_LABELS.live, count: liveReports.length },
    ...sources
      .filter((id) => id !== "live")
      .map((id) => ({
        id,
        label: REPORT_SOURCE_LABELS[id],
        count: all.filter((r) => r.source === id).length,
      })),
...
function buildCommandBar(
  ...
  return {
    ...
    approved: liveReports.filter((r) => r.status === "approved").length,
    pendingReview: liveReports.filter((r) => r.status === "pending_review").length,
    legacyArchived: legacyArchive.length,
    lastSync: snapshot.refreshedAt,
  };
}

export interface BuildReportsCenterOptions {
  source?: ReportsCenterSource | "all";
  agentTab?: ReportsCenterAgentTab | "all";
}
...
export async function buildReportsCenterPayload(
  ...
  options: BuildReportsCenterOptions = {},
): Promise<ReportsCenterPayload> {
  const brainCards = brainReports.map((r) => reportToCenterReport(r, tasks));
  ...
  const liveBrain = brainCards.filter((r) => r.source === "live");
  const legacyBrain = brainCards.filter((r) => r.source !== "live");
  const demoReports = brainReports.length === 0 ? buildDemoReports(tasks) : [];
  const ceoBriefings = buildCeoBriefings(liveBrain, snapshot);

  const liveReports = [...liveBrain, ...commerceReports].sort(
  ...
  const sourceFilter = options.source ?? "live";
  const agentTabFilter = options.agentTab ?? "all";

  let visibleReports =
    sourceFilter === "all" ? [...liveReports, ...legacyArchive] : liveReports;
  if (sourceFilter !== "all" && sourceFilter !== "live") {
    visibleReports = legacyArchive.filter((r) => r.source === sourceFilter);
  }

  if (agentTabFilter !== "all") {
    visibleReports = visibleReports.filter((r) => r.agentTab === agentTabFilter);
  }

  return {
    commandBar: buildCommandBar(liveReports, legacyArchive, snapshot),
    agentTabs: buildAgentTabFilters(
      sourceFilter === "live" ? liveReports : [...liveReports, ...legacyArchive],
    ),
    sourceFilters: buildSourceFilters(liveReports, legacyArchive),
    reports: visibleReports,
    legacyArchive,
    activityFeed: buildActivityFeed(snapshot, liveReports),
