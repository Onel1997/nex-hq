...
export class MemoryDeterministicJobRepository implements DeterministicJobRepository {
  ...
  async list(scope: WorkspaceScope, filters: { projectId?: string } = {}) { return [...this.jobs.values()].filter((job) => job.workspaceId === scope.workspaceId && (!filters.projectId || job.productionProjectId === filters.projectId)).map((job) => structuredClone(job)); }
  ...
  async markUnknown(scope:WorkspaceScope,id:string,reason:string,now:string){const job=this.require(scope,id);if(job.status!=="running")throw new Error("Job is not running");const next={...job,status:"unknown_outcome" as const,unknownOutcomeReason:reason,reconciliationState:"required" as const,completedAt:now,updatedAt:now};this.jobs.set(id,next);return structuredClone(next);}
}
