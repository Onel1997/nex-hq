...
export function emptyMarketPrintIntelligence(): MarketPrintIntelligence {
  return {
    profile: MARKETPRINT_PROFILE,
    premiumProductTypes: getPremiumProducts(),
    embroideryProductTypes: getEmbroideryProducts(),
...
export function safeBuildMarketPrintIntelligence(
  products: ShopifyKnowledgeProduct[] = [],
): MarketPrintIntelligence {
  try {
    return buildMarketPrintIntelligence(products);
  } catch (error) {
    console.error("[MarketPrint] Failed to build intelligence, using fallback.", error);
    return emptyMarketPrintIntelligence();
  }
}
