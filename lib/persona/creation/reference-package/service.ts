...
import { reconcileReferencePackageState } from "./reconcile-reference-package-state";
import {
...
async function generateOneAngle(
  ...
  let attempt = await repo.createAttempt(scope, {
    ...
    replacement_for_asset_id: input.replacement?.forAssetId ?? null,
    replacement_for_slot: input.replacement?.forSlot ?? null,
    replacement_candidate: input.replacement != null,
  });
  ...
  try {
    ...
    const isReplacement = input.replacement != null;
    const accepted = isReplacement
      ? angleOk && identityEligibleForReview
      : identityOk && angleOk;
    ...
    const notes = buildReferencePackageAssetNotes({
      ...
      replacementForAssetId: input.replacement?.forAssetId ?? null,
      replacementForSlot: input.replacement?.forSlot ?? null,
      replacementCandidate: isReplacement ? true : null,
    });
    ...
    attempt = await repo.updateAttempt(scope, attempt.id, {
      ...
      replacement_for_asset_id: input.replacement?.forAssetId ?? null,
      replacement_for_slot: input.replacement?.forSlot ?? null,
      replacement_candidate: isReplacement,
...
/** Phase 2.3D.10 — paid regeneration as replacement candidate (incumbent stays active). */
export async function generateOneAngleForReplacement(
  scope: WorkspaceScope,
  input: {
    personaId: string;
    sessionId: string;
  ...
  const attempts = await pkgRepo(input.deps).listAttemptsForPersona(
    scope,
    input.personaId,
  );
  const directionPlan = resolveProviderDirectionPlan(attempts, input.slot);
  return generateOneAngle(scope, {
    personaId: input.personaId,
    sessionId: input.sessionId,
    masterReferenceId: input.masterReferenceId,
    masterStoragePath: input.masterStoragePath,
    masterMimeType: input.masterMimeType,
    slot: input.slot,
    directionPlan,
    replacement: {
      forAssetId: input.replacementForAssetId,
      forSlot: input.slot,
    },
    deps: input.deps,
  });
...
export async function getReferencePackageStatus(
  ...
  const attempts = await repo.listAttemptsForPersona(scope, personaId);
  const assets = await personaRepo().listReferenceAssets(scope, personaId);
  const referencePackageState = reconcileReferencePackageState({ attempts, assets });
  const coverage = resolveReferencePackageSlotCoverage({ attempts, assets });

  const slots: ReferencePackageSlotView[] = referencePackageState.slots.map((row) => {
    const plan = resolveProviderDirectionPlan(attempts, row.slot);
    const legacy = coverage.slots.find((s) => s.slot === row.slot)!;
    return {
      slot: row.slot,
      ...
      acceptedAssetId: row.usable ? row.activeAssetId : null,
      pendingReplacementAssetId: row.pendingReplacementAssetId,
      usable: row.usable,
      replacementState: row.replacementState,
      replacementPredecessorMissing: row.replacementPredecessorMissing,
  ...
  return {
    ...
    session,
    slots,
    referencePackageState,
    acceptedCount: referencePackageState.acceptedCount,
    requiredCount: referencePackageState.requiredCount,
    referencePackageReady: referencePackageState.referencePackageReady,
