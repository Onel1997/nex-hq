...
function collectErrorText(error: unknown): string {
  if (error instanceof Error) {
    return [
      error.message,
      error.name,
      error.cause ? collectErrorText(error.cause) : "",
    ]
      .filter(Boolean)
      .join(" ");
  }
...
export function isProviderQuotaError(error: unknown): boolean {
  ...
  return Boolean(text) && PROVIDER_QUOTA_PATTERNS.some((pattern) => pattern.test(text));
}

/** Infrastructure failures for which local workspace fallback is safe. */
export function isRecoverableWorkspaceError(error: unknown): boolean {
