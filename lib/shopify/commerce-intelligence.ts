...
function normalizeCommerceDebug(
  ...
  return {
    ...
    lineItemsCount: partial.lineItemsCount ?? 0,
    totalRevenue: partial.totalRevenue ?? 0,
    firstOrderDate: partial.firstOrderDate ?? null,
    lastOrderDate: partial.lastOrderDate ?? null,
    tokenScopes: partial.tokenScopes,
    appAccessScopes: partial.appAccessScopes,
    hasReadOrders: partial.hasReadOrders,
    hasReadAllOrders: partial.hasReadAllOrders,
...
export async function fetchCommerceIntelligence(
  ...
  const debug =
    normalizeCommerceDebug(resolution.debug) ?? (await probeShopifyOrdersAccess());
...
export async function loadCommerceIntelligenceSafe(
  ...
  try {
    const resolution = await commerceHistoryProvider.resolve(knowledge);
    const debug = normalizeCommerceDebug(resolution.debug);
