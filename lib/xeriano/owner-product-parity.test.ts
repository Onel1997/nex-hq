...
test("Creative and UGC Owner modes keep customer markup defaults unchanged", () => {
  ...
  assert.match(creativeCss, /\.creative-studio-shell\.is-owner-product-mode \.cs-quick-bar/);
  assert.match(ugcCss, /\.ugc-video-studio-shell\.is-owner-product-mode \.uv-generate-bar/);
  assert.match(creative, /props\.customerMode \? `Generieren · \$\{customerCredits\} Credits` : "Generieren"/);
  assert.match(ugc, /props\.customerMode&&customerCredits!==null\?`Generieren · \$\{customerCredits\} Credits`:"Generieren"/);
